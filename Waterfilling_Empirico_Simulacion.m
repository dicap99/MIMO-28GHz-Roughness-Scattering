% Waterfilling_Empirico_Simulacion.m
clc; clear; close all;
...
% Ángulos de incidencia medidos
theta_deg = [1.95; 3.90; 5.86; 7.82; 9.79; 11.78; 13.77; 15.79; 17.83; 19.89];
...
%% 2. CARGAR RESULTADOS DE POTENCIA DISPERSADA DESDE CSV
T_emp = readtable('Resultados_Potencia_Dispersada1.csv');

% Asegurar que coinciden los ángulos de la simulación
theta_deg = T_emp.Angulo;
num_angulos = length(theta_deg);

% Función para convertir de dBm a lineal (mW)
% Notar que 10^(-Inf) resulta correctamente en 0 mW
dBm_to_mW = @(x) 10.^(x / 10);

% --- Extracción Dinámica de la Tabla ---
...
%% 4. SIMULACIÓN Y WATERFILLING (ÁNGULO POR ÁNGULO)
% Parámetros Modelo 1
G_sys = 53.25;
...
% Valores empíricos de rugosidad en eje X (mm) indicados por el usuario
...
for i = 1:num_angulos
    ...
    P_alloc_emp = zeros(4, 3); % 4 flujos, 3 tamaños
    for k = 1:3
        if k == 1 % Pequeñas
    ...
    % Superposición empírica Modelo 1
    ...
    title(sprintf('Modelo 1 vs Empírico - \\theta_i = %.2f^\\circ', ti_deg), 'Interpreter', 'tex');
    ...
    % Superposición empírica Modelo 2
    plot(x_emp, (P_alloc_emp(1, :)/Pt_W)*100, 'o', 'MarkerSize', 8, 'MarkerFaceColor', c1, 'Color', 'k');
    plot(x_emp, (P_alloc_emp(2, :)/Pt_W)*100, 's', 'MarkerSize', 8, 'MarkerFaceColor', c2, 'Color', 'k');
    plot(x_emp, (P_alloc_emp(3, :)/Pt_W)*100, '^', 'MarkerSize', 8, 'MarkerFaceColor', c3, 'Color', 'k');
    plot(x_emp, (P_alloc_emp(4, :)/Pt_W)*100, 'd', 'MarkerSize', 8, 'MarkerFaceColor', c4, 'Color', 'k');
    ...
    ylim([0, 105]); xlim([0, 10]);
    title(sprintf('Modelo 2 vs Empírico - \\theta_i = %.2f^\\circ', ti_deg), 'Interpreter', 'tex');
    ylabel('Pot. Asignada (%)'); xlabel('Rugosidad RMS s (mm)', 'Interpreter', 'tex');
    
    % Leyenda Global (Solo en la primera figura para no saturar si quieren)
    % O en todas, pero abajo
